...
def dashboard_leads(request):
    ...
    if fonte:
        leads_filtrados = leads_filtrados.filter(fonte__icontains=fonte)
    if horario:
        leads_filtrados = leads_filtrados.filter(melhor_horario_contato=horario)
    ...
    leads_por_fonte = (
        leads_filtrados.values("fonte").annotate(total=Count("id")).order_by("-total")
    )
    leads_por_status = (
        leads_filtrados.values("status").annotate(total=Count("id")).order_by("-total")
    )
    ultimos_leads = leads_filtrados.order_by("-data_criacao")[:5]

    fontes_labels = json.dumps(
        [item["fonte"] or "Não especificado" for item in leads_por_fonte]
    )
    fontes_data = json.dumps([item["total"] for item in leads_por_fonte])
    status_labels = json.dumps(
        [lead.get("status").replace("_", " ").title() for lead in leads_por_status]
    )
    ...
    contexto = {
        ...
        "horario_choices": Lead._meta.get_field("melhor_horario_contato").choices,
...
def lead_listar(request):
    ...
    if curso:
        leads = leads.filter(curso_interesse=curso)
    if contato:
        leads = leads.filter(contato__icontains=contato)
    if status:
        leads = leads.filter(status=status)
    if fonte:
        leads = leads.filter(fonte__icontains=fonte)
    if horario:
        leads = leads.filter(melhor_horario_contato=horario)
...
def update_lead_status(request):
    try:
        ...
        return JsonResponse(
            {
                "status": "success",
                "message": f"Status do lead {lead_id} atualizado para {new_status}.",
            }
        )
